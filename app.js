...
async function resetPassword() {
    ...
    const enteredOTP =
        prompt("Enter the OTP sent to you");
...
async function sendResetOTP() {
    ...
    if (!username || !contact) {
        alert("Please enter your username and email/phone.");
        return;
    }

    const button =
        document.querySelector("#forgotStep1 button");
    ...
    try {
        ...
        const response = await fetch(
            "https://ftslcifbzohhgljqcgus.supabase.co/functions/v1/send-reset-otp",
            {
                method: "POST",
...
async function verifyResetOTP() {
    ...
    const otp =
        document.getElementById("fpOTP").value.trim();
    ...
    const button =
        document.querySelector("#forgotStep2 button");
    ...
    try {
        ...
        const response = await fetch(
            "https://ftslcifbzohhgljqcgus.supabase.co/functions/v1/verify-reset-otp",
            {
                method: "POST",
...
async function changeResetPassword() {
    ...
    const newPassword =
        document.getElementById("fpNewPassword").value.trim();

    const confirmPassword =
        document.getElementById("fpConfirmPassword").value.trim();
    ...
    if (!resetUserId) {
        alert("Reset session is missing. Please start again.");
        return;
    }
    ...
    const button =
        document.querySelector("#forgotStep3 button");
    ...
    try {
        ...
        const response = await fetch(
            "https://ftslcifbzohhgljqcgus.supabase.co/functions/v1/rapid-responder",
            {
                method: "POST",

                headers: {
                    "Authorization":
                        "Bearer sb_publishable_xF3b2JXH72rrwvJ2MCfOuA_Lm_C-a2D",

                    "apikey":
                        "sb_publishable_xF3b2JXH72rrwvJ2MCfOuA_Lm_C-a2D",
...
// =========================================================
// MODERN STUDENT TERMINAL REPORT
// =========================================================
...
async function generateReportWithTerm(
    ...
    if (schoolError) {
        console.error("School information error:", schoolError);
    }

    let logo = schoolData?.schoolLogo || "";
...
">
    ...
    "></div>
        ...
        <div style="
            text-align:center;
            padding-bottom:12px;
            border-bottom:3px solid #00075D;
        ">
    ...
    let teacherName =
        getClassTeacherName(s.studentclass);


    // =========================================================
    // LOAD CLASS TEACHER SIGNATURE
    // =========================================================
...
/* =========================================================
   ...
   =========================================================
   Uses the existing global `students` array populated by
   your Supabase `loadStudents()` function.

   Fee statements are currently stored in localStorage.
   ========================================================= */
...
const FEE_STORAGE_KEY = "schoolFeeStatements";

let feeCalculationInitialized = false;
let feeFilterInitialized = false;
...
function loadFeeStatements() {

    try {

        const saved = localStorage.getItem(FEE_STORAGE_KEY);
        ...
        feeStatements = Array.isArray(parsed)
            ? parsed
            : [];
...
function saveFeeData() {
    ...
    try {
        ...
        localStorage.setItem(
            FEE_STORAGE_KEY,
            JSON.stringify(feeStatements)
        );

    } catch (error) {

        console.error(
            "Error saving fee statements:",
            error
        );
...
async function loadFeeStudents() {
    ...
    if (!select) {
        console.warn("Fee student select not found.");
        return;
    }


    /*
       Use the EXISTING loadStudents() function.

       Your loadStudents() already gets students from:
       ...
       Therefore we do NOT use localStorage for students.
    */
    ...
    try {
        ...
        console.error(
            "Unable to load students for Fees:",
            error
        );
    ...
    /*
       Make sure the main system has returned
    ...
    */
    ...
    students.forEach((student, index) => {
        ...
        /*
           Prefer Supabase database ID.

           If database ID does not exist,
           use studentid.
        */
        ...
        option.textContent =
            `${fullName} — ${studentId} — ${studentClass}`;


        /*
           Keep useful student information
           attached to the option.
        */

        option.dataset.studentIndex =
            index;
    ...
    console.log(
        `Fees: ${students.length} students loaded successfully.`
    );
}
...
function findFeeStudent(studentValue) {
    ...
    /*
       ...
       We use the existing global `students`
       array populated by loadStudents().
    */
...
/* =========================================================
   GET SELECTED FEE STUDENT
   ========================================================= */

function getSelectedFeeStudent() {
...
function getFeeStudentName(studentValue) {

    const student =
        findFeeStudent(studentValue);
...
function formatFeeMoney(amount) {
...
function calculateFeeTotal() {
    ...
    const tuition =
        getFeeNumber("feeTuition");


    const books =
        getFeeNumber("feeBooks");
...
function setupFeeCalculation() {
    ...
    feeCalculationInitialized = true;
...
function saveFeeStatement() {
    ...
    /*
       Get the real student object
       from the existing students array.
    */
    ...
    if (!student) {
        ...
        alert(
            "The selected student could not be found. Please reload the Fees page and try again."
        );
    ...
    const studentName =
        getFeeStudentName(studentValue);
    ...
    /*
       ...
       a statement for the same term.
    */

    const existing =
        feeStatements.find(
            statement =>
                String(statement.studentId) ===
                    String(
                        student.studentid ||
                        student.id
                    ) &&
    ...
    if (existing) {
        ...
        const confirmUpdate =
            confirm(
                `${studentName} already has a fee statement for ${formatTerm(term)}.\n\nDo you want to create another statement?`
            );
    ...
    /*
       Use the student's actual Student ID
       as the primary fee record identifier.
    */
    ...
    /*
       Add newest statement to beginning.
    */

    feeStatements.unshift(
        statement
    );
    ...
    */
    ...
    alert(
        `Fee statement created successfully for ${studentName}.`
    );
}
...
function getFeeStatus(statement) {
...
function updateFeeStatuses() {
    ...
    if (!Array.isArray(feeStatements)) {
        feeStatements = [];
    }


    feeStatements.forEach(
        statement => {
...
/* =========================================================
   CREATE FEE RECORD HTML
   ========================================================= */

function createFeeRecordHTML(
    statement
) {

    const status =
        getFeeStatus(statement);
    ...
    return `
        ...
        <div
            class="fee-record-item"
            data-status="${escapeFeeHTML(status)}"
        >

            <div class="fee-record-main">

                <div class="fee-record-student-icon">
                    <i class="fas fa-user-graduate"></i>
                </div>


                <div class="fee-record-student">
            ...
            <div class="fee-record-meta">

                <span class="fee-status ${escapeFeeHTML(status)}">
            ...
            <div class="fee-record-actions">
                ...
                ${
                    status !== "paid"
                        ? `
                            <button
                                type="button"
                                onclick="recordFeePayment('${escapeFeeHTML(statement.id)}')"
                ...
                <button
                    type="button"
                    onclick="viewFeeStatement('${escapeFeeHTML(statement.id)}')"
                >
                ...
                <button
                    ...
                    onclick="deleteFeeStatement('${escapeFeeHTML(statement.id)}')"
...
function recordFeePayment(
    ...
    if (!statement) {
        ...
        alert(
            "Fee statement could not be found."
        );
    ...
    if (balance <= 0) {
        ...
        alert(
            "This fee statement has already been fully paid."
        );
    ...
    const input =
        prompt(
            `Student: ${statement.studentName}\n\nOutstanding balance: ${formatFeeMoney(balance)}\n\nEnter payment amount:`
        );
    ...
    if (amount > balance) {
        ...
        alert(
            `Payment cannot exceed the outstanding balance of ${formatFeeMoney(balance)}.`
        );
    ...
    */
    ...
    alert(
        `Payment of ${formatFeeMoney(amount)} recorded successfully.`
    );
}
...
function viewFeeStatement(
            ...
            ? statement.payments
                .map(
                    payment => {
                        ...
                        return (
                            `${formatFeeMoney(payment.amount)} - ${date}`
                        );
    ...
    alert(
        ...
        `Student ID: ${statement.studentId || "N/A"}\n` +

        `Class: ${statement.studentClass || "N/A"}\n` +

        `Term: ${formatTerm(statement.term)}\n` +

        `Academic Year: ${statement.academicYear || "N/A"}\n\n` +

        `Tuition: ${formatFeeMoney(statement.tuition)}\n` +

        `Books: ${formatFeeMoney(statement.books)}\n` +

        `ICT: ${formatFeeMoney(statement.ict)}\n` +

        `Other Fees: ${formatFeeMoney(statement.other)}\n` +

        `Total: ${formatFeeMoney(statement.total)}\n\n` +

        `Paid: ${formatFeeMoney(statement.amountPaid)}\n` +

        `Outstanding: ${formatFeeMoney(statement.outstanding)}\n` +

        `Status: ${formatTerm(statement.status)}\n` +

        `Due Date: ${statement.dueDate || "Not specified"}\n\n` +
...
/* =========================================================
   DELETE FEE STATEMENT
   ========================================================= */

function deleteFeeStatement(
    statementId
) {
    ...
    const confirmDelete =
        confirm(
            `Delete the fee statement for ${statement.studentName}?\n\nThis action cannot be undone.`
        );
...
function updateFeeStatistics() {
    ...
    const totalStatements =
        feeStatements.length;


    const totalPaid =
        feeStatements.reduce(
            (sum, statement) =>
                sum +
                Number(
                    statement.amountPaid || 0
    ...
    if (totalStatementsElement) {
        ...
        totalStatementsElement.textContent =
            totalStatements;
    }
...
async function showFeesPage() {
    ...
    */
    ...
    /*
       Load students from the existing
       Supabase student system.
    */
...
async function initializeFees() {
    ...
    */
    ...
    /*
       Do NOT call loadFeeStudents()
       blindly here before the application
       has loaded currentUser.

       If currentUser is already available,
       load the students.
    */
